...
class MorphableModel:
    ...
    def __init__(self, path, textureResolution = 256, trimPca = False, landmarksPathName = 'landmark_62_mp.txt', device='cuda'):
        ...
        with open(pathLandmarks, 'r') as f:
            lines = f.readlines()

        # Extract landmarks association and landmarks mask
        landmarksAssociation = []
        landmarksMask = []

        for line in lines:
            parts = line.strip().split('\t\t')  # Split line by '\t\t'
            if len(parts) >= 2:
                landmarksMask.append(int(parts[0]))   # First column (index 0) as landmarks mask
                landmarksAssociation.append(int(parts[1]))  # Second column (index 1) as landmarks association

        # Convert lists to torch tensors
        landmarksAssociationTensor = torch.tensor(landmarksAssociation, dtype=torch.int64).to(device)
        landmarksMaskTensor = torch.tensor(landmarksMask, dtype=torch.int64).to(device)

        # Now use the tensors in your pipeline
        self.landmarksAssociation = landmarksAssociationTensor
        self.landmarksMask = landmarksMaskTensor
